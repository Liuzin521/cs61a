...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    max_number = max(player_score,opponent_score)
    min_number = min(player_score,opponent_score)
    d = min_number 
    
    while d > 0:
        if max_number % d == 0 and min_number % d == 0:
            if d >= 10:
                return True
        d -= 1
    
    if min_number == 0 or d == 0:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal and score1 < goal):
        if who == 0:
            ...
            score0 = score0 + take_turn(player0_num_dice,score1,dice) 
            if not extra_turn(score0,score1):
                who = other(who)
        else:
            player1_num_dice = strategy1(score1,score0)
            score1 = score1 + take_turn(player1_num_dice,score0,dice)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    number_dice = 1
    max_average = 0
    while number_dice < 11:
        score = make_averaged(roll_dice, trials_count)
        average = score(number_dice,dice)
