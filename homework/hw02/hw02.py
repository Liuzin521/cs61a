...
def pingpong(n):
    ...
    def helper(index, ppn, dir):
        if index == n:
            return ppn
        elif (index % 8 != 0) and num_eights(index) == 0:
...
def count_coins(total):
    ...
    def helper(total, current_coin):
        if total == 0:
            return 1
        if total < 0 or current_coin is None:
            return 0
        return helper(total - current_coin, current_coin) + helper(total, next_largest_coin(current_coin))
